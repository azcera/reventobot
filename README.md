...
Бот для управления семейным сервером Revento (заявки, капты, архивы, Telegram-интеграция).
...
- `src/features/` — **Основная бизнес-логика**, сгруппированная по доменам (`invite`, `capture`, `archive`).
- `src/services/` — Глобальные сервисы (PostgreSQL, Telegram API).
- `src/utils/` — Переиспользуемые утилиты (автоудаление, работа с датами).
- `src/events/` — Глобальные слушатели событий Discord (`interactionCreate`, `messageCreate`).
